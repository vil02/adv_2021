...
def find_path_do_expl_pair(in_data):
    ...
    def is_atom(in_node):
        return (
            isinstance(in_node, tuple)
            and isinstance(in_node[0], int)
            and isinstance(in_node[1], int)
        )
...
def solve_b(in_str):
    ...
    return max(
        max(magnitude(add_values(a, b)), magnitude(add_values(b, a)))
        for (a, b) in itertools.combinations(parse_input(in_str), 2)
    )
