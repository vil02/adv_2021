...
def _get_grid_cell_volume(in_all_uniuqe_coordinates, in_coord_num_list):
    def get_size(in_axis):
        cur_coords = in_all_uniuqe_coordinates[in_axis]
        return (
            cur_coords[in_coord_num_list[in_axis] + 1]
            - cur_coords[in_coord_num_list[in_axis]]
        )

    assert len(in_all_uniuqe_coordinates) == len(in_coord_num_list)
    return functools.reduce(
        lambda a, b: a * b, (get_size(_) for _ in range(len(in_coord_num_list)))
    )
